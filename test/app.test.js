...
test("agenda isola compromissos por usuario e notifica no horario", async (context) => {
  ...
  const created = await post(base, "/api/appointments", { date: "2026-07-19", time: "18:30", description: "Reuniao com cliente", notes: "Levar contrato", profile: "Casa" }, first.cookie);
  const company = await post(base, "/api/appointments", { date: "2026-07-20", time: "10:00", description: "Reuniao Empresa", notes: "Contrato", profile: "Empresa" }, first.cookie);
  const blockedCompany = await post(base, "/api/appointments", { date: "2026-07-20", time: "10:00", description: "Agenda Empresa", notes: "", profile: "Empresa" }, second.cookie);
  assert.equal(created.status, 201);
  assert.equal(created.body.profile, "Casa");
  assert.equal(company.status, 201);
  assert.equal(company.body.profile, "Empresa");
  assert.equal(blockedCompany.status, 402);
  const firstData = (await get(base, "/api/data", first.cookie)).body;
  assert.deepEqual(firstData.appointments.map((item) => item.profile).sort(), ["Casa", "Empresa"]);
